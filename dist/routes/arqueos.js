...
router.get('/', async (req, res) => {
    const { empresaId, usuarioId, rol } = req.user;
    try {
...
router.patch('/:id/aprobar', (0, auth_1.requireRol)('AUDITOR'), async (req, res) => {
    ...
    try {
        await db_1.default.query(`UPDATE arqueos
       SET estado_aprobacion=$1, aprobado_por=$2,
           fecha_aprobacion=NOW(), observacion_auditor=$3
       WHERE arqueo_id=$4 AND empresa_id=$5`, [estado, usuarioId, observacion || null, parseInt(req.params.id), empresaId]);
